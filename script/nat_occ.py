...
cas_orb = np.load('cas_no.npy')
cas_orb = np.asarray([cas_orb[0]]*2)
rdm1_high = onepdm(norb, restricted=False, bogoliubov=False, tmpDir='./node0')
rdm1_ao = np.zeros_like(rdm1_high)

with open('nat_occ.dat', 'w') as f:
    f.write('%10s %10s %10s \n'%('orb #', 'occ_a', 'occ_b'))
    occ_col = []
    for s in range(2):
        rdm1_ao[s] = mdot(cas_orb[s], rdm1_high[s], cas_orb[s].conj().T)
        occ = la.eigh(rdm1_ao[s])[0][::-1]
        occ_col.append(occ)
    for i in range(20):
        f.write("%10s %10.5f %10.5f \n"%(i, occ_col[0][i], occ_col[1][i]))

np.save('rdm1_lo.npy', rdm1_high)
np.save('rdm1_ao.npy', rdm1_ao)
